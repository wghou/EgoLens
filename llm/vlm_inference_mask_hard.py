...
class EgoAffordDataset(Dataset):
    ...
    def __init__(self, base_dir):
        ...
        self.question_template = (
            "Goal: '{main_task}'\n"
            "Please analyze the scene based on the image ({size}*{size}).\n"
            ...
            "\n"
            "IMPORTANT RULES:\n"
            "- Implicitly infer the next action step and do not output it explicitly.\n"
            ...
            "- Only output the current action step's components.\n"
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(output_dir, f"{args.type}_mask_hard.json")):
        with open(os.path.join(output_dir, f"{args.type}_mask_hard.json"), 'r') as f:
    ...
    with open(os.path.join(output_dir, f"{args.type}_mask_hard.json"), 'w') as f:
